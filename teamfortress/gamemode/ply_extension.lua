...
local sv_force_hl2_hull = CreateConVar("sv_force_hl2_hull", 0, {FCVAR_NOTIFY, FCVAR_REPLICATED})
...
function meta:ShouldUseDefaultHull()
	local force = sv_force_hl2_hull:GetInt()
	
	if force>0 then return true
	elseif force<0 then return false
	else
		return self:GetNWBool("IsHL2") or GetMapType(game.GetMap())=="hl2"
	end
end
